import { TRANSLITERATOR_MAPPINGS } from './mappings.js';
      ...
      switch (token.type) {
        ...
        case 'passthrough':
          if (/[0-9]/.test(token.char) && map.numerals) {
            output += map.numerals[parseInt(token.char, 10)];
          } else {
            output += token.char;
          }
          break;
...
const romanEngine = new OfflineTransliterator();

export function fromRoman(text) {
  const input = text ?? '';
  if (!input.trim()) {
    return { roman: input, devanagari: '', takri: '', warnings: [] };
  }
  return {
    roman: input,
    devanagari: romanEngine.transliterate(input, 'devanagari'),
    takri: romanEngine.transliterate(input, 'takri'),
    warnings: [],
  };
}
